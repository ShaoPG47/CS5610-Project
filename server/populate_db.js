...
//For docker: $ node server/populate_db "mongodb://localhost:27017/fake_so"
...
function answerCreate(text, ans_by, ans_date_time, question, comments) {
  let answerdetail = {text:text};
  if (ans_by != false) answerdetail.ans_by = ans_by;
  if (ans_date_time != false) answerdetail.ans_date_time = ans_date_time;
  if (question != false) answerdetail.question = question;
...
function userCreate(
  ...
  if (questions != false) useremail.questions = questions;
  if (answers != false) userdetail.answers = answers;
  if (qComments != false) userdetail.qComments = qComments;
  if (aComments != false) userdetail.aComments = aComments

  let user = new User(userdetail)
  return user.save()
}

function qCommentCreate(text, cmt_by, question) {
  qcmtdetail = {text: text}

  if (cmt_by != false) qcmtdetail.cmt_by = cmt_by;
  if (question != false) qcmtdetail.question = question

  let qcmt = new Qcomments(qcmtdetail)
  return qcmt.save()
}

function aCommentCreate(text, cmt_by, answer) {
  acmtdetail = {text: text}

  if (cmt_by != false) acmtdetail.cmt_by = cmt_by;
  if (answer != false) acmtdetail.answers = answer;

  let acmt = new Acomments(acmtdetail)
  return acmt.save()
}
...
const populate = async () => {
  ...
  let ac1 = await aCommentCreate("I tried, it didn't work", "rookieman1");
  ...
  let a8 = await answerCreate('Store data in a SQLLite database.', 'ihba001', new Date('2023-03-22T21:17:53'), [], [ac1]);

  let qc1 = await qCommentCreate("I have the same problem!", "fruitloop")
